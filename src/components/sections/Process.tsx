...
export default function Process() {
  ...
  return (
    <section id="process" className="relative py-24 md:py-36 bg-background border-t border-foreground/[0.04] px-6 md:px-12 overflow-hidden select-none">
      <div className="max-w-7xl mx-auto w-full">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-24 items-start">
          ...
          <div className="lg:col-span-5 lg:sticky lg:top-36 space-y-6">
            ...
            <h2 className="text-4xl md:text-5xl font-display font-extrabold tracking-tight text-foreground leading-tight">
            ...
            <div className="hidden lg:block pt-8 text-[10px] font-mono text-muted-text/30">
              [ HOVER OR TAB TO EXPAND THE PIPELINE ]
            </div>
          </div>

          {/* Right Column: Expanding Steps */}
          <div className="lg:col-span-7 divide-y divide-foreground/[0.06] border-t border-b border-foreground/[0.06]">
            {steps.map((step, index) => {
              ...
              return (
                <div
                  key={step.number}
                  className="py-8 md:py-10 transition-all duration-300 relative group cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent rounded-lg px-2"
                ...
                >
                  ...
                  <div className="flex items-start justify-between gap-6 relative z-10">
                    <div className="flex gap-6 md:gap-10 items-start">
                      ...
                      <div className="space-y-2">
                        <h3 className="text-lg md:text-xl font-display font-bold tracking-tight text-foreground group-hover:text-accent transition-colors duration-300">
                          {step.title}
                        </h3>
                    ...
                    <div className="relative w-4 h-4 mt-2 hidden sm:block">
                      <span className="absolute inset-y-0 left-1.5 w-0.5 bg-foreground/40 group-hover:bg-accent transition-all duration-300 group-hover:rotate-90" />
                      <span className={`absolute inset-x-0 top-1.5 h-0.5 bg-foreground/40 group-hover:bg-accent transition-all duration-300 ${isExpanded ? "rotate-90 scale-0" : ""}`} />
                    </div>
                  </div>
                  ...
                  >
                    <div className="pl-12 md:pl-16 grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-foreground/[0.04] text-xs text-muted-text leading-relaxed">
                      
                      {/* Left: Detail Description */}
                      <div className="space-y-4">
                        <p>{step.description}</p>
                        <div className="flex flex-col gap-1.5 pt-2">
                          <span className="text-[9px] font-mono tracking-widest uppercase text-foreground/50">Deliverables</span>
                          <div className="flex flex-wrap gap-1.5">
                            {step.deliverables.map((item) => (
                              <span key={item} className="px-2 py-0.5 rounded border border-card-border bg-card-bg/30 text-[8px] font-mono">
                                {item}
                              </span>
                            ))}
                          </div>
                      ...
                      <div className="p-4 rounded-lg bg-card-bg/30 border border-card-border flex flex-col justify-between">
                        <div className="space-y-2">
                        ...
                        <span className="text-[8px] font-mono text-foreground/20 self-end mt-4">
                          [ SYNCED PIPELINE ]
                        </span>
                      </div>
