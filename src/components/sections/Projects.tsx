...
export default function Projects() {
  return (
    <section id="work" className="relative py-24 md:py-36 bg-dark-bg px-6 md:px-12 overflow-hidden select-none">
      <div className="max-w-7xl mx-auto w-full">
        {/* Section Header */}
        <div className="flex flex-col md:flex-row md:items-end justify-between border-b border-white/[0.06] pb-12 mb-16 md:mb-24 gap-6">
              ...
              >
                ...
                <div className="w-full lg:w-5/12 flex flex-col justify-center space-y-6">
                  ...
                  <div className="space-y-3">
                    <h3 className="text-3xl md:text-4xl font-display font-bold tracking-tight text-white hover:text-accent transition-colors duration-300">
                  ...
                  <div className="flex flex-wrap gap-2 pt-2">
                    {project.tags.map((tag) => (
                      <span
                        key={tag}
                        className="px-3 py-1 text-[10px] font-mono tracking-wider text-muted-text rounded-full border border-white/[0.06] bg-white/[0.01]"
                      >
                        {tag}
                      </span>
                    ))}
                  ...
                  <div className="pt-4">
                    <Magnetic range={40} strength={0.3}>
                      <a
                        href={project.link}
                        className="group inline-flex items-center gap-2 text-xs font-mono uppercase tracking-widest text-white hover:text-accent transition-all duration-300"
                      >
                ...
                {/* 2. Custom Rendered Visual Mockup */}
                <motion.div 
                  whileHover={{ y: -8, scale: 1.005 }}
                  transition={{ type: "spring", stiffness: 150, damping: 15 }}
                  className="w-full lg:w-7/12 aspect-[4/3] rounded-2xl border border-white/[0.08] bg-card-bg/40 backdrop-blur-sm overflow-hidden relative group cursor-pointer shadow-2xl hover:border-white/[0.12] transition-all duration-500"
                >
                  <a href={project.link} className="block w-full h-full relative">
                    <div className="absolute inset-0 bg-gradient-to-t from-dark-bg/60 to-transparent z-10 opacity-60 group-hover:opacity-20 transition-all duration-500" />
                      ...
                      {project.previewType === "analytics" && (
                        <div className="w-full h-full rounded-lg border border-white/[0.04] bg-neutral-950 overflow-hidden shadow-inner flex flex-col relative">
                          ...
                          <div className="h-8 border-b border-white/[0.04] px-4 flex items-center gap-1.5 shrink-0 bg-white/[0.01]">
                            ...
                            <div className="text-[9px] font-mono text-muted-text ml-auto">aether_dashboard.ts</div>
                          </div>
                          {/* Workspace */}
                          <div className="flex-grow p-4 grid grid-cols-3 gap-3 relative grid-bg">
                            ...
                            <div className="col-span-2 rounded-md border border-white/[0.04] bg-white/[0.02] p-3 flex flex-col justify-between h-[120px] z-10">
                              <span className="text-[8px] font-mono text-muted-text uppercase tracking-widest">Active Yield Rate</span>
                              <span className="text-xl font-display font-semibold text-white tracking-tight">$42,912.45</span>
                            ...
                            <div className="col-span-1 rounded-md border border-white/[0.04] bg-white/[0.02] p-3 flex flex-col justify-between h-[120px] z-10">
                              <span className="text-[8px] font-mono text-muted-text uppercase tracking-widest">Risk Profile</span>
                              ...
                              <span className="text-[9px] font-mono text-white text-center">75% SECURE</span>
                            </div>
                            {/* Card C */}
                            <div className="col-span-3 rounded-md border border-white/[0.04] bg-white/[0.02] p-3 flex items-center justify-between z-10">
                              <div className="flex gap-2 items-center">
                                ...
                                <span className="text-[8px] font-mono text-muted-text uppercase">Node-US-East synced</span>
                              </div>
                              <span className="text-[8px] font-mono text-accent uppercase tracking-widest">Live Updates</span>
                            </div>
                          </div>
                        </div>
                      )}
                        ...
                        <div className="w-full h-full rounded-lg border border-white/[0.04] bg-neutral-950 overflow-hidden shadow-inner flex flex-col relative">
                          <div className="flex-grow p-6 flex flex-col justify-between relative">
                            ...
                            <div className="grid grid-cols-5 gap-4 items-start">
                              ...
                              <div className="col-span-2 border-r border-white/[0.05] pr-4 h-full flex flex-col justify-between">
                                <div className="space-y-1">
                                  <div className="text-[8px] font-mono text-white/40 uppercase">Architecture</div>
                                  <div className="text-[11px] font-display font-medium text-white">MAISON D'OR</div>
                                </div>
                                <div className="pt-8 text-[7px] font-mono text-muted-text leading-normal">
                                  Paris, FR &mdash;<br/>Photographed 2026
                                </div>
                              </div>
                                ...
                                <div className="w-full h-full bg-gradient-to-tr from-neutral-900 to-neutral-800 flex items-center justify-center relative">
                                  ...
                                  <div className="absolute inset-0 flex items-center justify-center">
                                    <div className="w-[85%] h-[85%] border border-white/5 flex items-center justify-center">
                                      <span className="text-[10px] font-mono text-white/10">03_PLATE_A</span>
                                    </div>
                                  </div>
                                </div>
                              </div>
                            ...
                            <div className="flex justify-between items-end border-t border-white/[0.04] pt-4 mt-6">
                              <span className="text-[8px] font-mono text-muted-text">COLLECTION V &bull; ARCHIVES</span>
                              <span className="text-[8px] font-mono text-white/70">1 / 18 PLATES</span>
                            </div>
                          </div>
                        </div>
                      )}
